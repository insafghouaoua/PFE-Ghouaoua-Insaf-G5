...
<head>
    ...
    <title>Commandes partagee</title>
    	<!-- Favicon -->
...
<section class="home-section">
    <table>
        ...
        <tbody>
            <?php
            $commande = $pdo->query('SELECT c.commande_id, c.type, c.utilisateur_id, cp.produit_id, cp.quantite, cp.prix_vente, (cp.quantite * cp.prix_vente) AS prix_total, c.nom, c.prenom, c.telephone, c.email, c.wilaya, c.adresse_postale, c.cree_le FROM commande AS c INNER JOIN commande_produit AS cp ON c.commande_id = cp.commande_id WHERE c.type="partagee" ORDER BY c.commande_id');
            if ($commande->rowCount() == 0) {
                echo '<tr>';
                echo '<td colspan="16">Aucune commande partagée.</td>';
                echo '</tr>';
            } else {

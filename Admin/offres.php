...
<head>
    ...
    <title>Offres et participations</title>
    	<!-- Favicon -->
...
<table>
  ...
  <?php
  $offre = $pdo->query('SELECT *, offre_achat_partage.nom AS offre_achat_partage_nom  FROM offre_achat_partage ORDER BY produit_id');

if ($offre->rowCount() == 0) {
  echo '<tr>';
  echo '<th colspan="10"><h2>Offres<h2></th>';
  echo '</tr>';
  echo '<tr>';
  echo ' <th>Offre ID</th>';
  ...
  echo '<td colspan="10">Aucun offre</td>';
  echo '</tr>';
...
while($ligne = $offre->fetch()){ 
  ...
  echo '<tr>';
  echo '<td colspan="10"><h2>Offre<h2></td>';
  echo '</tr>';
  echo '<tr>';
  echo ' <th>Offre ID</th>';
  echo ' <th>Produit ID</th>';
